...
**Légendes (décision Eddy, 2026-09-08) : elles suivent le site.** Une légende ne nomme jamais une autre salle que le club de destination du site, et ne prétend jamais qu'une photo a été prise dans un lieu où elle ne l'a pas été. Elle dit ce qu'on voit et la marque : « Travail au sac — Boxing Center », « La cage MMA — Boxing Center ». Quand la photo vient bien du club de destination, on le nomme (« Boxing Center Portet-sur-Garonne »). Le champ `lieu` du manifeste garde la vérité en interne ; il ne s'affiche pas. Un `alt` décrit toujours la scène, jamais un lieu.
...
Manifeste `medias.ts` : slug, source, lieu de prise de vue (interne, non affiché), alt (ce qu'on voit), légende, paires N&B/couleur si elles existent. **Aucune mention de photographe** nulle part (décision Eddy) ; les mentions légales sont **identiques à celles de Colomiers** (éditeur, établissements, hébergeur Vercel, nature du site, propriété intellectuelle, liens sortants — SIRET et directeur de publication restent à renseigner, comme sur Colomiers).
